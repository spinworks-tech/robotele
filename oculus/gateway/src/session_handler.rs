...
//! `roboprotocol_core::profile` types. Mirrors operator-console's
//! `session_handler.rs`: this gateway auto-accepts the full advertised
//! profile exactly like operator-console does (v0, no narrowing UI), even
//! though it never actuates any of it -- SESSION_ACCEPT is required to
//! reach the Operating phase regardless of whether we intend to send
//! Channel B commands.
